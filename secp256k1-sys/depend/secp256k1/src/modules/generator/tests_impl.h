...
void test_generator_api(void) {
    ...
    rustsecp256k1_v0_2_0_generator gen;
    int32_t ecount = 0;
    ...
    rustsecp256k1_v0_2_0_testrand256(key);
    rustsecp256k1_v0_2_0_testrand256(blind);

    CHECK(rustsecp256k1_v0_2_0_generator_generate(none, &gen, key) == 1);
    CHECK(ecount == 0);
    CHECK(rustsecp256k1_v0_2_0_generator_generate(none, NULL, key) == 0);
    CHECK(ecount == 1);
    CHECK(rustsecp256k1_v0_2_0_generator_generate(none, &gen, NULL) == 0);
    CHECK(ecount == 2);

    CHECK(rustsecp256k1_v0_2_0_generator_generate_blinded(sign, &gen, key, blind) == 1);
    CHECK(ecount == 2);
    CHECK(rustsecp256k1_v0_2_0_generator_generate_blinded(vrfy, &gen, key, blind) == 0);
    CHECK(ecount == 3);
    CHECK(rustsecp256k1_v0_2_0_generator_generate_blinded(none, &gen, key, blind) == 0);
    CHECK(ecount == 4);
    CHECK(rustsecp256k1_v0_2_0_generator_generate_blinded(vrfy, NULL, key, blind) == 0);
    CHECK(ecount == 5);
    CHECK(rustsecp256k1_v0_2_0_generator_generate_blinded(vrfy, &gen, NULL, blind) == 0);
    CHECK(ecount == 6);
    CHECK(rustsecp256k1_v0_2_0_generator_generate_blinded(vrfy, &gen, key, NULL) == 0);
    CHECK(ecount == 7);

    CHECK(rustsecp256k1_v0_2_0_generator_serialize(none, sergen, &gen) == 1);
    CHECK(ecount == 7);
    CHECK(rustsecp256k1_v0_2_0_generator_serialize(none, NULL, &gen) == 0);
    CHECK(ecount == 8);
    CHECK(rustsecp256k1_v0_2_0_generator_serialize(none, sergen, NULL) == 0);
    CHECK(ecount == 9);

    CHECK(rustsecp256k1_v0_2_0_generator_serialize(none, sergen, &gen) == 1);
    CHECK(rustsecp256k1_v0_2_0_generator_parse(none, &gen, sergen) == 1);
    CHECK(ecount == 9);
    CHECK(rustsecp256k1_v0_2_0_generator_parse(none, NULL, sergen) == 0);
    CHECK(ecount == 10);
    CHECK(rustsecp256k1_v0_2_0_generator_parse(none, &gen, NULL) == 0);
    CHECK(ecount == 11);
...
void test_shallue_van_de_woestijne(void) {
    ...
    static const rustsecp256k1_v0_2_0_ge_storage results[32] = {
    ...
    rustsecp256k1_v0_2_0_ge_storage ges;
    int i, s;
    for (i = 1; i <= 16; i++) {
        rustsecp256k1_v0_2_0_fe_set_int(&fe, i);

        for (s = 0; s < 2; s++) {
            if (s) {
                rustsecp256k1_v0_2_0_fe_negate(&fe, &fe, 1);
                rustsecp256k1_v0_2_0_fe_normalize(&fe);
            }
            shallue_van_de_woestijne(&ge, &fe);
            rustsecp256k1_v0_2_0_ge_to_storage(&ges, &ge);

            CHECK(memcmp(&ges, &results[i * 2 + s - 2], sizeof(rustsecp256k1_v0_2_0_ge_storage)) == 0);
        }
    }
}
...
void test_generator_generate(void) {
    ...
    rustsecp256k1_v0_2_0_scalar_set_b32(&sc, s, NULL);
    for (i = 1; i <= 32; i++) {
        memset(v, 0, 31);
        v[31] = i;
        CHECK(rustsecp256k1_v0_2_0_generator_generate_blinded(ctx, &gen, v, s));
        rustsecp256k1_v0_2_0_generator_load(&ge, &gen);
        rustsecp256k1_v0_2_0_ge_to_storage(&ges, &ge);
        CHECK(memcmp(&ges, &results[i - 1], sizeof(rustsecp256k1_v0_2_0_ge_storage)) == 0);
        CHECK(rustsecp256k1_v0_2_0_generator_generate(ctx, &gen, v));
        rustsecp256k1_v0_2_0_generator_load(&ge, &gen);
        rustsecp256k1_v0_2_0_ge_to_storage(&ges, &ge);
        CHECK(memcmp(&ges, &results[i - 1], sizeof(rustsecp256k1_v0_2_0_ge_storage)) == 0);
    }
...
void test_generator_fixed_vector(void) {
    ...
    rustsecp256k1_v0_2_0_generator parse;

    CHECK(rustsecp256k1_v0_2_0_generator_parse(ctx, &parse, two_g));
    CHECK(rustsecp256k1_v0_2_0_generator_serialize(ctx, result, &parse));
    CHECK(memcmp(two_g, result, 33) == 0);

    result[0] = 0x0a;
    CHECK(rustsecp256k1_v0_2_0_generator_parse(ctx, &parse, result));
    result[0] = 0x08;
    CHECK(!rustsecp256k1_v0_2_0_generator_parse(ctx, &parse, result));
}
