...
impl Generator {
    ...
    pub fn from_slice(bytes: &[u8]) -> Result<Self, Error> {
        let mut public_key = ffi::PublicKey::new();
...
impl<C: Signing> Secp256k1<C> {
    /// Blind a [`Tag`] using a given blinding factor.
    pub fn blind(&self, tag: Tag, blinding_factor: SecretKey) -> Generator {
        let mut generator = ffi::PublicKey::default();
